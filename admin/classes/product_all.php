...
class Products
{
    ...
    public function productUpdate($data, $files, $id)
    {
        ...
        if ($productName == "" || $catId == "" || $brandid == "" || $price == "" || $body == "") {
            ...
            if ($getData) {
                $resultCheck = $getData->fetch_assoc();
                if (empty($file_name)) {
                    $unique_image = $resultCheck['image'];
                }
                if (empty($related_file_name)) {
                    $related_unique_image = $resultCheck['relatedImage'];
                }
            }
        }
    ...
    public function delCatById($id)
    {
        ...
        if ($getData) {
            while ($delImg = $getData->fetch_assoc()) {
                $delLink1 = "../assets/img/product/" . $delImg['image'];
                unlink($delLink1);
                $delLink2 = "../assets/img/product/" . $delImg['relatedImage'];
                unlink($delLink2);
            }
        }
